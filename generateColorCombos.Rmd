...
nzpBanding <- nzpBanding %>%
  filter(!is.na(spp)) %>%
  select(spp, colorComboL, colorComboR) %>%
  distinct

# Process NN banding file

nnBanding <- nnBanding %>%
  select(siteIDCapture, sppCapture:colorComboCapture) %>%
  mutate(regionSub = siteIDCapture %>%
           str_sub(start = -3, end = -2)) %>%
  filter(regionSub %in% c('DC', 'MD', 'VA')) %>%
  filter(!is.na(sppCapture)) %>%
  select(spp = sppCapture, colorCombo = colorComboCapture) %>%
  distinct
...
```{r conditionsFrame, echo=FALSE,results="hide", include=FALSE}
